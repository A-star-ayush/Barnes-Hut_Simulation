...
package object barneshut {
  ...
  case class Fork(
    ...
    val centerX: Float = (nw.centerX + ne.centerX + sw.centerX + se.centerX)/4f
    val centerY: Float = (nw.centerY + ne.centerY + sw.centerY + se.centerY)/4f
  ...
  extends Quad {
    ...
    val massX: Float = bodies.foldLeft(0f){ case(s, b) => s + b.x*b.mass } / mass
    val massY: Float = bodies.foldLeft(0f){ case(s,b) => s + b.y*b.mass } / mass 
    ...
    def insert(b: Body): Quad = {
      if(size <= minimumSize) Leaf(centerX, centerY, size, b +: bodies)
        ...
        val by2 = size/2
        val by4 = size/4
        var f = Fork(Empty(centerX - by4, centerY - by4, by2), Empty(centerX + by4, centerY - by4, by2), 
                     Empty(centerX - by4, centerY + by4, by2), Empty(centerX + by4, centerY + by4, by2))
        for(x<-bodies) f = f.insert(x)  // don't forget to catch the return value.. can use foldLeft to avoid confusion
        f = f.insert(b)
        f
      }
    }
    ...
    def updated(quad: Quad): Body = {
      ...
      def addForce(thatMass: Float, thatMassX: Float, thatMassY: Float): Unit = {
        ...
        if (dist > 1f) {
          ...
          val xn = (thatMassX - x) / dist  // cos theta
          val yn = (thatMassY - y) / dist  // sin theta
